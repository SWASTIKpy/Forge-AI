...
function parseGeneratedResponse(output: string): GeneratedResponse {
  ...
  // Sanitize any hallucinated file:/// URLs to prevent Sandpack SecurityError crashes
  const sanitizedJson = json.replace(
    /file:\/\/\/[^\s"'<)\\]+/g,
    "https://picsum.photos/200"
  );

  return JSON.parse(sanitizedJson) as GeneratedResponse;
}
...
function generationErrorMessage(error: unknown): string {
  ...
  if (/api.?key|unauthenticated|permission denied|forbidden/i.test(message)) {
    return "Gemini rejected the API key. Check GEMINI_API_KEY and its API permissions.";
  }
  return "Generation failed. Check the development server logs for details.";
}
...
11. Never mutate Error objects directly (e.g. \`error.message = "..."\`). This will cause a "read-only" runtime error in the preview environment. Create new Error objects instead if you need to throw or set state.
12. NEVER use \`file:///\` URLs or local file paths for images or assets. This causes a SecurityError in the Sandpack iframe and crashes the preview. Always use valid \`https://\` URLs, data URIs, or placeholder services (like picsum).`;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
          ...
          try {
            geminiStream = await ai.models.generateContentStream({
              ...
              config: {
                ...
                responseJsonSchema: GENERATED_APP_SCHEMA,
              },
            });
            break;
          } catch (error) {
        ...
        if (!geminiStream) {
          throw new Error("Gemini did not return a generation stream.");
        }
          ...
          for (const part of parts) {
            ...
            if (part.thought) {
              ...
              if (!startedGenerating) {
                ...
                enqueue(sseEvent("status", { message: "Writing code..." }));
              }
              // Actual JSON output
        ...
        try {
          parsed = parseGeneratedResponse(accumulated);
        } catch {
          console.error(
            "[gen-ai-code] invalid model JSON:",
